...
class RcpBridge {
  ...
  Future<String> _getLibraryPath(String libraryName) async {
    if (Platform.isAndroid) {
      ...
      return path.join(DynamicLibrary.executable().toString().split('/').sublist(0, 5).join('/'), 
  ...
  Map<String, dynamic> _processResult(Pointer<RcpResult> resultPtr) {
    ...
    try {
      ...
      if (result.error_message != Pointer<Utf8>.fromAddress(0)) {
        response['error'] = result.error_message.toDartString();
      }
