...
pub unsafe extern "C" fn connect_to_server(host: *const c_char, port: i32) -> i32 {
    let host_str = {
        if host.is_null() {
...
/// Calling with an invalid or null pointer may result in undefined behavior.
pub unsafe extern "C" fn get_available_apps(count: *mut i32) -> *mut *mut c_char {
...
/// Calling with an invalid or null pointer may result in undefined behavior.
pub unsafe extern "C" fn launch_app(app_id: *const c_char) -> i32 {
    let app_id_str = {
        if app_id.is_null() {
            return -1; // Null pointer error
        }
